...
namespace AoC2018
{
    class Day2
    {
        ...
        public string CalcB()
        {
            var allEntries = _data.Split(new[] {'\r', '\n'}, StringSplitOptions.RemoveEmptyEntries);
            
            var set = new HashSet<string>();
            foreach (var str in allEntries)
            {
                for (int i = 0; i < str.Length - 1; i++)
                {
                    ...
                    var test = str.Remove(i, 1);
                    if (set.Add(test) == false)
                        return test;
                }
            }
        ...
        private static string _data = @"efmyhuxcqqldtwjzvisepargvo
